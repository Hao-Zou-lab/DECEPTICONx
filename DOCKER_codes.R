...
mapGenes<-function(mydata) {
  HGNC <- read.table("./HGNC_genenames_20170418.txt", 
...
quanTlseq<-function(currsig, currmix, scaling, method) {
...
celldensities <- function(DCres){
  ...
  imageinfo <- read.csv("D:/projects/6-deconvolution/totalcells.txt", header=FALSE, sep="\t", stringsAsFactors = FALSE)
